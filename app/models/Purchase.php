...
final class Purchase
{
  ...
  public static function list(array $filters = []): array
  {
    ...
    if ($q !== '') {
      $where[] = '(c.id_compra = :idq OR c.nota LIKE :nota OR pr.nombre LIKE :nota)';
  ...
  {
    $st = db()->prepare("
    SELECT c.*, u.nombre AS usuario, pr.nombre AS proveedor
    FROM compras c
    INNER JOIN usuarios u ON u.id_usuario = c.id_usuario
    LEFT JOIN proveedores pr ON pr.id_proveedor = c.id_proveedor
  ...
  {
    $st = db()->query("SELECT id_proveedor, nombre FROM proveedores ORDER BY nombre ASC");
    return $st->fetchAll();
  }
...
VALUES (NULL, :u, :prov, NOW(), 'REGISTRADA', :nota)
  ...
  public static function cancel(int $idCompra, int $idUsuario, string $nota = 'Compra eliminada'): void
  {
    ...
    if ((string) $compra['estado'] === 'ANULADA')
      throw new Exception('Esta compra ya está ANULADA.');

    $items = self::items($idCompra);
    if (!$items)
      throw new Exception('Esta compra no tiene detalle.');
